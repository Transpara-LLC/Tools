...
function getInputs { 
    ...
    $global:newSQLDBName = [Microsoft.VisualBasic.Interaction]::InputBox("Enter the new SQL DB name ", "SQL DB Name Request")
...
function createRemoteContextServices {
    if(![System.IO.File]::Exists($remoteContextPath)){
        return
    }
...
$remoteContextPath = "$global:pathToNewInstance\Remote Context Services" 
